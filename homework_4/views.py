...
def product_update_form(request, product_id):
    ...
    return render(request, 'homework_4/product_update.html', {'form': form})
...
def product_update_id_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            response = redirect(f'/homework_4/product_update/{prod_id}')
            return response
    else:
        form = ProductChoiceForm()
    return render(request, 'homework_4/product_update_id.html', {'form': form})


def index(request):
    return render(request, "homework_4/index.html")
